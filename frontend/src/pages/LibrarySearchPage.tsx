...
import { getLibraryTags, searchLibraryAlbums, type TagItem } from '../api/library'
...
export default function LibrarySearchPage() {
  ...
  // 搜索查询（keepPreviousData 避免切换 tag 时白屏闪烁）
  const hasFilter = query.trim() !== '' || selectedTags.length > 0
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['library-search', query, selectedTags, page],
    queryFn: () => searchLibraryAlbums({ q: query, tags: selectedTags, page }),
    enabled: hasFilter,
    placeholderData: keepPreviousData,
  })
  ...
  const pagination: PaginationInfo | null = hasFilter && data
    ? {
        current: page,
        total: count,
    ...
    <div className="relative z-10 mx-auto mt-24 w-full max-w-7xl px-4 pb-16 sm:px-6 lg:px-10">
      ...
      <div className="mb-8 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">在本地藏书阁中按名称、标签精确筛选</p>
        </div>
      </div>
      ...
      <div className="mx-auto mb-8 w-full max-w-2xl">
        ...
        {selectedTags.length > 0 && (
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            {selectedTags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-1.5 rounded-full border border-indigo-200/60 bg-indigo-50/70 px-3 py-1.5 text-xs font-bold text-indigo-600 shadow-sm dark:border-indigo-500/30 dark:bg-indigo-950/40 dark:text-indigo-400"
              >
                {tag}
                <button type="button" onClick={() => removeTag(tag)} className="ml-0.5 rounded-full p-0.5 transition-colors hover:bg-indigo-200/60 dark:hover:bg-indigo-800/60">
        ...
        <button
          ...
          className="flex items-center gap-2 text-sm font-bold text-slate-600 transition-colors hover:text-indigo-600 dark:text-slate-300 dark:hover:text-indigo-400"
        >
          ...
          <svg
            className={`h-4 w-4 transition-transform duration-300 ${showTagPanel ? 'rotate-180' : ''}`}
        ...
        <div
          ...
          style={{
            gridTemplateRows: showTagPanel ? '1fr' : '0fr',
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-xs font-semibold transition-all duration-200 active:scale-95 ${
                        active
                          ? 'border-indigo-400/60 bg-indigo-500/90 text-white shadow-md shadow-indigo-500/20'
                          : 'border-white/50 bg-white/40 text-slate-600 hover:border-indigo-300/60 hover:bg-indigo-50/50 hover:text-indigo-600 dark:border-white/10 dark:bg-slate-700/40 dark:text-slate-300 dark:hover:border-indigo-500/40 dark:hover:text-indigo-400'
                      }`}
                    >
                      {item.tag}
                    </button>
      ...
      {/* 切换筛选条件时的轻量 loading 指示（保留旧数据，顶部叠加指示条） */}
      {hasFilter && !isLoading && showFetching && (
      ...
      {hasFilter && !isLoading && albums.length === 0 && (
